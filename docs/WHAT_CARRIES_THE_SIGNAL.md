...
near 0.5). The posterior collapsed to zero: 0.0016 ± 0.0005, 95% HDI
[0.0008, 0.0028] ([`.audit/n_exponent_posterior_analysis.md`](../.audit/n_exponent_posterior_analysis.md)).
...
The n_exponent posterior is the thinnest fit in the repo: 1 chain × 500
samples, so no between-chain R-hat backs it. The conclusion is corroborated by
the EIV bake-off null, but the ablation should be re-run at ≥2 chains (ideally
4 × 1000) before this note is cited externally — tracked in #44.
