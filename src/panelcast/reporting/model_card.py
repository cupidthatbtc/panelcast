...
def create_default_model_card_data(descriptor=None) -> ModelCardData:
    ...
    return ModelCardData(
        ...
        architecture_summary=(
            ...
            "Mathematical form:\n"
            "- y_ij ~ StudentT(df=4, mu_ij, sigma_i) (likelihood family "
            "configurable via --likelihood-family)\n"
            ...
            "Since 0.5.0 the likelihood operates on the offset-logit transformed "
            "target by default (--target-transform offset_logit), and since 0.6.0 "
            "the default feature block includes a stacked-GBM offset (gbm_offset) "
            "with genre-level pooling where the dataset provides a group column. "
            "The per-run values are recorded in the hyperparameters table."
        ),
        priors_description=(
        ...
        limitations=[
            ...
            (
                ...
                "candidate fix — an offset-logit target transform — is the default since "
                "0.5.0 (paired held-out ELPD +22.2 +/- 4.5 over identity on the corrected "
                "estimator, seed-stable), yet it does not move the skewness/max/q90 pins — "
                "it only trades the q50 pin for a q10 pin — so the structural mismatch "
                "remains. Alternative likelihood families (beta, skew-Student-t, "
                "skew-normal, split-normal, beta-binomial, mixture; selectable via "
                "--likelihood-family) have been implemented and evaluated; none moves the "
                "skewness/max pins, so Student-t remains the default. The symmetric "
                "likelihood's point accuracy and 95% interval calibration are unaffected."
            ),
            (
                "**Soft-clip at [0, 100] applies only under --target-transform identity.** "
                "With the 0.5.0 offset_logit default the bounds hold by construction and no "
                "clip is applied. Under the identity transform (the former default, still "
                "selectable) the bounded target meets a symmetric likelihood and soft_clip "
                "compresses both tails simultaneously."
            ),
