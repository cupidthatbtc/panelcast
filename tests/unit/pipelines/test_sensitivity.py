...
# --- from unit/pipelines/test_sensitivity_new.py ---


@pytest.fixture
def passing_convergence_new():
    return ConvergenceDiagnostics(
        rhat_max=1.001,
        ess_bulk_min=2500,
        ess_tail_min=2200,
...
def _make_fit_result_new(idata):
    return FitResult(
        mcmc=MagicMock(),
        idata=idata,
        divergences=0,
...
class TestAggregateSensitivityResultsCRPS:
    ...
    def test_crps_metric_populates_columns(self, mock_crps, passing_convergence_new):
        """CRPS metric aggregation should include mean_crps and n_obs."""
        results = {
            "v1": SensitivityResult(
                name="v1",
                config={},
                convergence=passing_convergence_new,
                crps=mock_crps,
            ),
        }
    ...
    def test_crps_metric_none_crps(self, passing_convergence_new):
...
class TestAggregateSensitivityResultsConvergence:
    ...
    def test_convergence_metric_includes_diag_fields(self, passing_convergence_new):
...
class TestAggregateSensitivityResultsELPD:
    ...
    def test_elpd_sorted_descending(self, passing_convergence_new):
...
class TestRunPriorSensitivityWithCoefVars:
    ...
    def test_coefficient_vars_passed_to_extract(
        self, monkeypatch, simple_idata_new, passing_convergence_new
    ):
        """coefficient_vars should be passed to extract_coefficient_summary."""
        fit_result = _make_fit_result_new(simple_idata_new)
        monkeypatch.setattr(
            "panelcast.pipelines.sensitivity.fit_model",
            lambda *a, **kw: fit_result,
        )
        monkeypatch.setattr(
            "panelcast.pipelines.sensitivity.check_convergence",
            lambda *a, **kw: passing_convergence_new,
        )
...
class TestRunThresholdSensitivityDefaults:
    ...
    def test_default_mcmc_config(self, monkeypatch, simple_idata_new, passing_convergence_new):
        """When mcmc_config is None, default MCMCConfig is used."""
        configs_used = []

        def fake_fit(model, args, config=None, progress_bar=True):
            configs_used.append(config)
            return _make_fit_result_new(simple_idata_new)
...
class TestCreateOatSummaryTableNew:
    ...
    def test_eligible_sorted_by_abs_delta(self, passing_convergence_new):
    ...
    def test_no_loo_for_variant(self, passing_convergence_new, mock_loo_new):
    ...
    def test_convergence_flag_values(self, passing_convergence_new):
...
class TestSensitivityResult:
    ...
    def test_all_fields_set(self, passing_convergence_new, mock_loo_new, mock_crps):
