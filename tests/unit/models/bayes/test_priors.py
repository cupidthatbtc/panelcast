...
class TestPriorConfigSerialization:
    ...
    def test_field_count(self):
        """PriorConfig should have exactly 71 fields.
        ...
        hs_slab_scale, hs_slab_df) + 3 for the period-effects gate
        (period_effects, period_constraint, sigma_period_scale) + 2 for the
        per-group variance gate (group_variance, tau_group_sigma_scale) + 2
        for the skew-normal population seam (entity_effect_prior_type,
        entity_skew_alpha_scale) + 2 for the innovation-shape seam
        (rw_innovation_type, rw_skew_alpha_scale) + 1 for boundary
        censoring (censor_at_bounds).
        """
        assert len(fields(PriorConfig)) == 71
