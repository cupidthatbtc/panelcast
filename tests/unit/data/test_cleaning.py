...
# =============================================================================
# filter_for_user_score_model tests
# =============================================================================


class TestFilterForUserScoreModel:
    """Tests for filter_for_user_score_model."""

    def test_drops_missing_identifiers(self):
        df = pd.DataFrame(
            {
                "Artist": ["A", " ", "C"],
                "Album": ["Alpha", "Beta", pd.NA],
                "User_Score": [80.0, 85.0, 90.0],
                "User_Ratings": [100, 120, 200],
            }
        )
        filtered = filter_for_user_score_model(df, min_ratings=10)
        assert len(filtered) == 1
        assert filtered.iloc[0]["Artist"] == "A"

    def test_drops_missing_user_score(self):
        df = pd.DataFrame(
            {
                "Artist": ["A", "B"],
                "Album": ["X", "Y"],
                "User_Score": [80.0, pd.NA],
                "User_Ratings": [100, 100],
            }
        )
        filtered = filter_for_user_score_model(df, min_ratings=10)
        assert len(filtered) == 1

    def test_drops_out_of_range_score(self):
        df = pd.DataFrame(
            {
                "Artist": ["A", "B"],
                "Album": ["X", "Y"],
                "User_Score": [80.0, 150.0],
                "User_Ratings": [100, 100],
            }
        )
        filtered = filter_for_user_score_model(df, min_ratings=10)
    ...
    def test_drops_below_min_ratings(self):
        df = pd.DataFrame(
            {
                "Artist": ["A", "B"],
                "Album": ["X", "Y"],
        ...
        df = pd.DataFrame(
            {
                ...
                "Album": ["X", "Y", "Z"],
                "User_Score": [80.0, 85.0, 90.0],
                "User_Ratings": [100, 200, 300],
            }
        )
        filtered = filter_for_user_score_model(df, min_ratings=10)
        assert len(filtered) == 3

    def test_negative_user_score_excluded(self):
        df = pd.DataFrame(
            {
                "Artist": ["A"],
                "Album": ["X"],
...
# =============================================================================
# filter_for_critic_score_model tests
# =============================================================================


class TestFilterForCriticScoreModel:
    """Tests for filter_for_critic_score_model."""

    def test_drops_missing_critic_score(self):
        df = pd.DataFrame(
            {
                "Artist": ["A", "B"],
                "Album": ["X", "Y"],
                "Critic_Score": [80.0, pd.NA],
                "Critic_Reviews": [10, 10],
            }
        )
        filtered = filter_for_critic_score_model(df, min_reviews=1)
        assert len(filtered) == 1

    def test_drops_out_of_range_critic_score(self):
        df = pd.DataFrame(
            {
                "Artist": ["A", "B"],
                "Album": ["X", "Y"],
                "Critic_Score": [80.0, 110.0],
                "Critic_Reviews": [10, 10],
            }
        )
        filtered = filter_for_critic_score_model(df, min_reviews=1)
    ...
    def test_drops_below_min_reviews(self):
        df = pd.DataFrame(
            {
                "Artist": ["A", "B"],
                "Album": ["X", "Y"],
                "Critic_Score": [80.0, 85.0],
                "Critic_Reviews": [5, 0],
            }
        )
        filtered = filter_for_critic_score_model(df, min_reviews=1)
